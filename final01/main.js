...
d3.csv("data.csv").then(misDatos=>{
    ...
    misDatos.map(d=>{
     ...
     var Gisele= misDatos.filter(d => {
      return d.name == "Gisele"
      })
      ...
      .attr("class", 'gisselleBündchen')
      .attr("height", d=> alto-margen.superior-margen.inferior -y(d.age))
      .attr("y", d=>y(d.age))  //donde empieza la barra
      ...
      grupoElementos.datum(misDatos).append('path')
       .attr("id", "misDatos")
       .attr("d", d3.line()
            .x(d=>x(d.year))
            .y(d=>y(ageD(d.year))
